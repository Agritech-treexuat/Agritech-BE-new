...
exports.addServiceTemplate = async (req, res) => {
  try {
    ...
    const { square, expectDeliveryPerWeek, expectedOutput, expectDeliveryAmount, price, herbMax, leafyMax, rootMax } =
      req.body
    ...
    res.status(201).json({
      message: 'ServiceTemplate created successfully',
      serviceTemplate: savedServiceTemplate,
      allServiceTemplates
    })
  } catch (error) {
    console.error(error)
...
exports.updateServiceTemplate = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: 'ServiceTemplate updated successfully',
      serviceTemplate: updatedServiceTemplate,
      allServiceTemplates
    })
  } catch (error) {
    console.error(error)
...
exports.getFarmServiceRequests = async (req, res) => {
  try {
    ...
    const requestsAllInformation = await Promise.all(
...
exports.updateServiceRequestStatus = async (req, res) => {
  try {
    ...
    const updatedServiceRequest = await ServiceRequest.findByIdAndUpdate(requestId, { $set: { status } }, { new: true })

    if (status === 'accepted' && old_status != 'accepted') {
      getPlantNames(
        updatedServiceRequest.herbListPlantId,
        updatedServiceRequest.leafyListPlantId,
        updatedServiceRequest.rootListPlantId
      ).then((names) =>
        initProjectGarden(farmId, names).then((projectIds) =>
          createGarden(
            farmId,
            updatedServiceRequest.clientId,
            projectIds,
            updatedServiceRequest.note,
            updatedServiceRequest.serviceTemplateId,
            updatedServiceRequest._id
          ).then((garden) => makePlantFarmingAndSeedByGardenId(garden._id))
        )
      )
    ...
    res.status(200).json({
      ...
      waitingServiceRequests: waitingServiceRequests
    })
  } catch (error) {
    console.error(error)
    res.status(500).json({ message: 'Internal Server Error' })
